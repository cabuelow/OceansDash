...
The Oceans Dashboard is an interactive application that displays indicator time-series data to monitor progress towards goals for nature, people, and climate. It has been developed as an R package {OceansDash} to facilitate robust documentation and long-term maintenance of the application using the {golem} framework ([read more here](https://engineering-shiny.org/build-app-golem.html)).

To install it, first open RStudio and install {devtools} if it is not installed already,
...
1. [Fork and clone](https://happygitwithr.com/fork-and-clone) the [github repository](https://github.com/cabuelow/OceansDash) or, if you already have a local {OceansDash} github repository, navigate to the folder on your computer. Then double-click on the .Rproj file `OceansDash.Rproj` to open it in RStudio
2. Start a new [development branch](https://happygitwithr.com/git-branches.html?q=branch#create-a-new-branch) in github
3. Make edits to R scripts in {OceansDash} while remembering 'pull, commit and push' regularly to your development branch. Click [**here**](#making-edits) for more information on what and how to edit
...
There are two main components to the dashboard application, the **user-interface** and the **server**. 

The **user-interface** is what what we see in the dashboard application. It takes inputs and displays outputs, and has been developed as a series of nested [modules](https://engineering-shiny.org/structuring-project.html?q=modules#using-shiny-modules). 

The **server** provides instructions for how the dashboard application reacts to inputs to the user-interface and displays outputs. For example, the server will contain {ggplot} code for creating outputs.

To edit the code base defining the dashboard application user-interface, modules, and server:      
1. Open the **'R/'** folder,
2. edit the user-interface tab panels in `app_ui.R`,
3. edit the modules (aka 'sub-apps') nested within `app_ui.R` in the following scripts: `mod_main.R` and `mod_ind_timeseries.R`

**TODO** [here insert a screenshot of the web app interface and highlight which ui and mod script correspond to which part of the user interface]

To edit data displayed in the user-interface:
...
If you want to deploy the app to a shinyapps.io server, run the development version as above and hit 'Publish' on the top right corner of the app as it is running.

**TODO** [here insert a screenshot of the 'Publish' button]

*There are many other files in the {OceansDash} package that are not discussed here. Those not mentioned are not necessary for app development, but you can find more information on what they do/how to use them [here](https://engineering-shiny.org/build-app-golem.html)*.
...
1.	Download the latest version of the data and save it in the **'data-raw/process-indicators/data-downloaded/effective-protection'** folder.  
3.	Open **“data-raw/process-indicators/scripts/country_eez_land.R”**. Replace the data path in Line 8 with the path to the new data and run the script. This will save the processed spatial data layer as a geopackage (.gpkg) and is used to process other spatial data layers (see steps for each indicator derived from spatial data below).

Spatial data on protected areas and other effective conservation measures (OECMs) are derived from [World Database on Protected Areas (WDPA)](https://www.protectedplanet.net/en/thematic-areas/wdpa?tab=WDPA) and the [World Database on Other Effective Area-based Conservation Measures (WDOECM)](https://www.protectedplanet.net/en/thematic-areas/oecms?tab=OECMs). We use the R package [{wdpar}](https://joss.theoj.org/papers/10.21105/joss.04594) to download and clean these spatial data layers, following best practice (Butchart et al., 2015; Hanson, 2022) which includes: a) excluding protected areas that do not have the status of “Designated”, “Inscribed” and “Established”, and areas that are UNESCO Biosphere Reserves, b) protected areas represented as points with a reported extent were buffered to a circular area using their reported extent, and c) overlapping protected areas are erased such that areas associated with more effective management categories (i.e. higher IUCN Protected Area Management Categories) and historical precedence are kept (Hanson, 2022).

To download the most recent protected area/OECM data and clean using {wdpar}:

1.	Open and run the script “data-raw/process-indicators/scripts/wdpa_data.R”. 

Once you have processed and cleaned the EEZ and protected area/OECM data (described above), you can update the **Effective protection** and **Carbon under effective protection** indicators following the steps below.

##### Effective Protection

Effective protection was calculated for three habitats: mangroves, coral reefs, and tidal (salt) marshes. Currently (as of September 2024), there is only a single time point available on extent of coral reefs and tidal marshes. Check for the latest data for the extent of each habitat at the following sources:
...
2.	Coral reefs: [Allen Coral Atlas (ACA) - Geomorphic Zonation and Benthic Habitat - v2.0](https://developers.google.com/earth-engine/datasets/catalog/ACA_reef_habitat_v2_0)
3.	Tidal Marshes: [Global tidal marshes 2020 dataset](https://zenodo.org/records/8420753)
...
5.	Open **“data-raw/process-indicators/scripts/protected_coral.R”**. Replace the data path in Line 31 with the path to the new data and run the script. This step can be skipped if no new data is available.
6.	Open **“data-raw/process-indicators/scripts/protected_marsh.R”**. Replace the data path in Line 25 with the path to the new data and run the script. This step can be skipped if no new data is available.
7.	Run **“data-raw/process-indicators/scripts/effective_protection_output.R”** to compile data. Run this step whenever the extent data is updated.

##### Carbon Under Effective Protection

Carbon under effective protection was calculated for mangrove and tidal marshes. Check for the latest data at the sources listed below:

1.	Above-ground biomass (AGB) in mangroves: [Global Mangrove Distribution, Aboveground Biomass, and Canopy Height](https://daac.ornl.gov/cgi-bin/dsviewer.pl?ds_id=1665)
2.	Soil organic carbon (SOC) in mangroves: [Global mangrove soil carbon data set](https://zenodo.org/records/7729492)
3.	Soil organic carbon (SOC) in tidal marsh: [Global soil organic carbon in tidal marshes version 1](https://zenodo.org/records/10940066)
