...
The above download and installation is not required if you simply want to use the Oceans Dashboard. The latest version is available on the [Shiny Server](https://cbuelow.shinyapps.io/OceansDash/)
...
1. Opening your local {OceansDash} github repo by double-clicking on the .Rproj file `OceansDash.Rproj`
...
5. Once you have made changes to any of the above, you can run the development version of the app via:

``` r
golem::run_dev()
```

6. If you want to deploy the app to a shinyapps.io server, run the development version as above and click the 'Publish' or 'Re-publish' button on the top right corner of the app as it is running.
...
- Declare packages that functions belong to explicitly with `::`, e.g. `dplyr::mutate` or you will get an error message saying that the function cannot be found

## Development TODO

- Tune server to improve speed - it seems to be the mapping filtering that is slowing things down, not the timeseries filtering
